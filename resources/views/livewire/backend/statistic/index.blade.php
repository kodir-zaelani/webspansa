                        ...
                        <div class="row">
                                ...
                                @if ($datastatistic->count())
                                    <div class="table-responsive">
                                        <table class="table mb-0 editable-table table-hover">
                                            ...
                                            <tbody>
                                                @foreach ($datastatistic as $no => $item)
                                                    <tr class="@if ($this->isChecked($item->id)) table-primary @endif">
                                                        <th class="text-right" scope="row">
                                                            {{ $no + $datastatistic->firstItem() }}</th>
                                                        <td>
                                                            <a title="Title"
                                                                href="{{$item->url}}"
                            ...
                            <div class="col-xl-6 col-md-6 col-lg-6 col-12">
                                 @if ($statusUpdate == true)
                                    @include('livewire.backend.statistic.edit')
                                @else
                                    @include('livewire.backend.statistic.create')
                                @endif
                            </div>
                        </div>
                        <div class="mt-3 row">
                            @if ($datastatistic->total() > 10)
                            <div class="col-12 ">
                                {{ $datastatistic->links() }}
                            </div>
                            @else
                            <div class="col-12 text-start small text-muted">
                                Page : {{ $datastatistic->currentPage() }} | Show {{ $datastatistic->count() }} data
                                of {{ $datastatistic->total() }}
                            </div>
                            @endif
                        </div>
                    </div>
